...
int findItems(int *st ,int qs , int qe ){
    int ans = st[max_nodes + qe];
    for(qs += max_nodes, qe += max_nodes + qe;qs<qe;qs>>=1,qe>>=1){
        if(qs&1){
            ans+= st[qs++];
        }
        if(qe&1){
...
int main(int argc, const char * argv[]) {
    freopen("/Users/shashanksaurabh/Desktop/Journey/D. Pashmak and Parmida's problem Latest/D. Pashmak and Parmida's problem Latest/input.txt", "r", stdin);
    freopen("/Users/shashanksaurabh/Desktop/Journey/D. Pashmak and Parmida's problem Latest/D. Pashmak and Parmida's problem Latest/output.txt", "w", stdout);
    map <int, int> hh,hh2;
    int n ,val , *st,ans = 0,arr1[100], arr2[100],arr[100];
    cin>>n;
    for(int i =0 ;i<n;i++){
        cin>>val;
        arr[i] = val;
